...
func Desktop() string {
	...
	return selectRandom(desktopAll)
}
...
func Windows() string {
	...
	return selectRandom(desktopWindows)
}
...
func Mac() string {
	...
	return selectRandom(desktopMac)
}
...
func Linux() string {
	...
	return selectRandom(desktopLinux)
}
...
func Android() string {
	...
	return selectRandom(mobileAndroid)
}

// select a random entry from a slice
func selectRandom(slice []string) string {
	// init rand
	r := rand.New(rand.NewSource(time.Now().UnixNano()))
	// get an random int <= len(slice)
	i := r.Intn((len(slice) - 0) + 0)
	// return a string
	return slice[i]
}
